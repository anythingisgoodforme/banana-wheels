...
const DEFAULT_BASE_SPEED = 7; // bumped player base speed
...
const CAR_SKINS_KEY = 'carGameSkins';
...
function logShopError(err) {
    const shop = document.getElementById('shop');
    if (shop) {
        shop.innerHTML = '<p style="color:#fff;font-weight:bold;">Shop failed to load.</p>';
    }
...
function loadMoney() {
    ...
    const legacy = localStorage.getItem('carGameMoney');
...
// Skins handling
const CAR_SKINS = [
    { id: 'classic', name: 'Classic Blue', price: 0, body: '#ffb347', outline: '#d15d00' },
    { id: 'red', name: 'Crimson Racer', price: 50, body: '#d62828', outline: '#8b0b0b' },
    { id: 'lime', name: 'Lime Flash', price: 100, body: '#8fdc4c', outline: '#4c8a1e' },
    { id: 'cyan', name: 'Cyan Glide', price: 150, body: '#4cc9f0', outline: '#1d79a0' },
    { id: 'gold', name: 'Golden King', price: 300, body: '#ffd166', outline: '#b77a00' },
];
let unlockedSkins = new Set([DEFAULT_SKIN_ID]);

function loadSkins() {
    const raw = localStorage.getItem(CAR_SKINS_KEY);
    if (!raw) return;
    try {
        const arr = JSON.parse(raw);
        unlockedSkins = new Set(arr);
        if (!unlockedSkins.has(player.skinId)) {
            player.skinId = DEFAULT_SKIN_ID;
        }
    } catch (e) {
        unlockedSkins = new Set([DEFAULT_SKIN_ID]);
    }
}

function saveSkins() {
    localStorage.setItem(CAR_SKINS_KEY, JSON.stringify([...unlockedSkins]));
}
...
function renderShop() {
    ...
    CAR_SKINS.forEach((skin) => {
        ...
        name.textContent = `${skin.name} (${skin.price === 0 ? 'Free' : '$' + skin.price})`;
        card.appendChild(name);
        const btn = document.createElement('button');
        if (unlockedSkins.has(skin.id)) {
            btn.textContent = skin.id === player.skinId ? 'Equipped' : 'Equip';
            btn.disabled = skin.id === player.skinId;
...
function openGodMode() {
    ...
    const choice = (prompt('Type "money", "highscore", or "skin"') || '').toLowerCase().trim();
    if (choice === 'money') {
        ...
        const ids = CAR_SKINS.map(s => s.id).join(', ');
        const skin = (prompt(`Enter skin id to unlock/select: ${ids}`) || '').toLowerCase().trim();
        const found = CAR_SKINS.find(s => s.id === skin);
        if (found) {
            unlockedSkins.add(found.id);
            saveSkins();
            player.skinId = found.id;
            renderShop();
            showPowerUpNotification(`Skin ${found.name} unlocked`);
        } else {
...
function startFunMusic() {
    ...
    const freqs = [523.25, 659.25, 783.99, 659.25]; // C5 E5 G5 E5
    let idx = 0;
    const beatMs = 320;
    ...
    const playFunBeat = () => {
        ...
        // Arp ping an octave higher
        const arp = audioCtx.createOscillator();
        arp.type = 'triangle';
        arp.frequency.value = f * 2;
        const ag = audioCtx.createGain();
        ag.gain.setValueAtTime(0, now + 0.08);
        ag.gain.linearRampToValueAtTime(0.08, now + 0.12);
        ag.gain.linearRampToValueAtTime(0, now + 0.22);
        arp.connect(ag);
        ag.connect(funGain);
        arp.start(now + 0.08);
        arp.stop(now + 0.25);
...
// Persist money on refresh/close
